...
public interface OutBoundTaskMapper extends BaseMapper<OutboundTask>{
     ...
     * @return
     */
    @Select("select CONCAT(CONCAT(\"'\",bill_no),\"'\") from (select TIMESTAMPDIFF(MINUTE,t.create_time,NOW()) overtime,t.* from outbound_task t where t.task_type=1 and t.task_state=0 and t.sfreq=0)a \r\n"+
            "WHERE a.overtime>#{overTime} ORDER BY a.overtime")
    List<String> getOutBoudTaskBillNoOverTimeStringList(@Param("overTime") long overTime);

    /**
     * 获取所有超时出库订单 按降序排序
     * @return
     */
    @Select("select CONCAT(CONCAT(\"'\",bill_no),\"'\") from (select TIMESTAMPDIFF(MINUTE,t.create_time,NOW()) overtime,t.* from outbound_task t where t.task_type=1 and t.task_state=0 and t.sfreq=1)a \r\n"+
            "WHERE a.overtime>#{overTime} ORDER BY a.overtime")
    List<String> getOutBoudTaskPickCodeBillNoOverTimeStringList(@Param("overTime") long overTime);
    /**
     * 获取所有超时出库订单 按降序排序
     * @return
     */
