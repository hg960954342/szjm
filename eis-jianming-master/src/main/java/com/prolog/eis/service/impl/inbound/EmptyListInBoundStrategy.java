    ...
    public void inbound(InboundTask inboundTask) {
        {
            ...
            //暂时定入库任务状态开始为0
            AgvStorageLocation distinPortInfo = inBoundContainerService.getInBound(coordinateAgv, PortInfoTaskTypeEnum.EMPTY_BUTTRESS);
            if(distinPortInfo==null){
                LogServices.logSysBusiness("空托叠入库没有找到可用入口");
                return;
            }
            //写托盘任务
