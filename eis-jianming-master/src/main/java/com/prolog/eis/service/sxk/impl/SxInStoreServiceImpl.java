...
public class SxInStoreServiceImpl implements SxInStoreService{
	...
	public Integer findLocationGroup(Integer layer, String taskProperty1, String taskProperty2, Integer originX,
		...
		// step1找相同属性的可用货位组
		List<InStoreLocationGroupDto> inStoreLocationGroupDtos = findSamePropertyLocationGroup(findSameTypeLocationGroup
				, taskProperty1, taskProperty2);
		if (inStoreLocationGroupDtos.size() == 0) {
			List<InStoreLocationGroupDto> emptyInStoreLocationGroupDto = ListHelper.where(findSameTypeLocationGroup, p->p.getContainerCount() == 0);
